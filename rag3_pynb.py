...
class ImprovedOCR:
    ...
    def extract_text_with_enhanced_ocr(pdf_path: str) -> List[Tuple[int, str]]:
        ...
        print("🔍 Converting PDF to images for enhanced OCR processing...")

        try:
            # Higher DPI for better text recognition
            images = convert_from_path(pdf_path, dpi=300, fmt='png')
            print(f"📄 Converted PDF to {len(images)} high-resolution images")
        ...
        for i, image in enumerate(images):
            ...
            try:
                ...
                if cleaned_text and len(cleaned_text.split()) > 5:  # Minimum viable text
                    ...
                    print(f"✅ Page {i+1}: Extracted {len(cleaned_text)} characters, {len(cleaned_text.split())} words")
                else:
                    print(f"⚠️ Page {i+1}: Insufficient text extracted")
...
Return only the 3 variations, one per line, without numbering or extra text."""
        ...
        for i, q in enumerate(queries):
            try:
                ...
                print(f"⚠️ Error in query variation {i}: {e}")
                continue
...
class AdvancedPDFQASystem:
    ...
    def extract_pages_with_quality_check(self, pdf_path: str) -> List[Tuple[int, str]]:
        ...
        print("📖 Attempting intelligent text extraction...")
        ...
        try:
            with open(pdf_path, 'rb') as file:
                ...
                if pages:
                    ...
                    print(f"📊 Text extraction stats: {len(pages)} pages, avg {avg_chars:.0f} chars/page, {avg_words:.0f} words/page")

                    # If quality is good enough, use text extraction
                    if avg_chars > 200 and avg_words > 30:
                        print("✅ High-quality text extraction successful!")
                        self.extraction_stats = {
                ...
                print("⚠️ Text extraction quality insufficient, switching to enhanced OCR...")

        except Exception as e:
            print(f"❌ Text extraction failed: {e}")
            print("🔄 Switching to enhanced OCR...")
    ...
    def upload_and_process_pdf(self):
        ...
        print(f"📄 Processing {pdf_path} with advanced extraction...")
        ...
        print(f"✅ Successfully extracted text from {len(pages)} pages using {self.extraction_stats['method']}")
        ...
        if self.total_chunks > 0:
            ...
            print(f"   🧩 {self.total_chunks} semantic chunks created")
            print(f"   🔧 Method: {self.extraction_stats['method']}")
            print("🎉 Ready for questions!\n")
        else:
            print("❌ Failed to create chunks from extracted text")

    def query(self, question: str, top_k: int = 8):
        """Answer questions with enhanced retrieval and generation."""
        if not self.is_processed:
            print("❌ Please process a PDF first using upload_and_process_pdf()")
            return
        ...
        print(f"\n📋 Retrieved content from {len(page_content)} page(s):")
        ...
        print(answer)
        print("="*70)
        print(f"📊 Answer based on {len(page_content)} relevant page(s)")
...
print("🚀 Advanced PDF QA System Ready!")
print("📋 Enhanced Features:")
print("  ✅ Smart text extraction with quality assessment")
print("  ✅ Enhanced OCR with image preprocessing")
print("  ✅ Semantic-aware chunking")
print("  ✅ Multi-query retrieval with re-ranking")
print("  ✅ Intelligent chunk combination")
